...
class Vgg19:
	...
	def __init__(self, vgg19_npy_path=None, trainable=True, dropout=0.5):
		if vgg19_npy_path is None:
			...
			path = os.path.join(path, "vgg19_train.npy")
			vgg19_npy_path = path
	...
	def test(self, batch_x, batch_y):
		...
		prediction, accuracy = self.sess.run(
			[tf.argmax(self.outputs), self.accuracy], fd)
	...
	def train(self, batch_x, batch_y, lr=1e-4, partial_train=True):
		fd = {}
		fd[self.input] = batch_x
